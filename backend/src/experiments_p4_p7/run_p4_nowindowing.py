"""
backend/src/experiments_p4_p7/run_p4_nowindowing.py

P4 -- No-Windowing. Variable tested: epoch length (one full 5-second epoch
vs. the standard five 1-second windows). Locked: 0.5-50Hz broadband filter,
SVM, subject-dependent architecture (like P3), no augmentation (E0
Baseline), phase_filter='all'.

Stage A (spot-check): subject S3 only, all 5 feature groups
    (time/hjorth/barlow/band_ratio/all) -> backend/models/weights/
    P4_NoWindowing/Spotcheck_S3_E0/{feature_group}/
Stage B (full-scale): all 12 subjects, using the feature group Stage A
    selected automatically (dataset_builders_ext.select_winning_feature_group)
    -> backend/models/weights/P4_NoWindowing/Fullscale_12Subj_E0/
    Auto-resume: subjects whose model file already exists are skipped
    (same pattern as models/run_e8_classical.py).

This extends the earlier single-subject/single-feature pilot in
models/run_p4_no_windowing.py (kept untouched as historical reference --
see backend/reports/p4_no_windowing_pilot_report.md and
p4_control_subsampled_report.md) to the full 5-feature spot-check and
12-subject grid, via NoWindowDatasetBuilder's proper subclassing rather than
that pilot's standalone duplicate-loop FullEpochDatasetBuilder. Writes to a
separate subfolder, so both coexist without collision.

Usage:
    cd backend/src/experiments_p4_p7
    python run_p4_nowindowing.py                # Stage A then Stage B
    python run_p4_nowindowing.py --stage a       # spot-check only
    python run_p4_nowindowing.py --stage b       # full-scale only (reads
                                                  # Stage A's saved summary)
"""
import os
import sys
import glob
...
from experiments_p4_p7.dataset_builders_ext import NoWindowDatasetBuilder, select_winning_feature_group
...
# Identical E0 baseline recipe used by P1/P2/P3 (0.5-50Hz broadband, no ICA,
# native 256Hz, no ERP cropping, no channel ablation, no augmentation).
...
REPORT_PATH = os.path.join(REPORTS_DIR, "P4_NoWindowing_report.md")
...
def load_no_window_epochs(subject_id, raw_data_dir):
    """Build one subject's no-windowing dataset via NoWindowDatasetBuilder.
    Returns (X_3d, y) with X_3d shape (N, channels, time), or (None, None)
    if the subject's raw data is unavailable."""
    builder = NoWindowDatasetBuilder(
        exp_id="RawBuild", processor_params=E0_PROCESSOR_PARAMS,
    ...
    # (N, time, channels) -> (N, channels, time), EEGFeatureExtractor's
...
def train_and_evaluate_one(X_3d, y, feat_group, weights_dir, subject_id, tag):
    """Shared split/extract/scale/train/evaluate/save routine for one
    (subject, feature_group) combination."""
...
def run_stage_a_spotcheck():
    print(f"\n{'=' * 70}\n P4 No-Windowing -- Stage A: Spot-check ({SPOTCHECK_SUBJECT}, {EXP_ID})\n{'=' * 70}")
    ...
    X_3d, y = load_no_window_epochs(SPOTCHECK_SUBJECT, raw_dir)
    if X_3d is None:
        raise RuntimeError(f"[P4 Stage A] No epochs extracted for subject {SPOTCHECK_SUBJECT}.")
    print(f"[INFO][P4-A] Full-epoch samples extracted: {X_3d.shape[0]} (shape per sample: {X_3d.shape[1:]})")
    ...
    for feat_group in FEATURE_GROUPS:
        ...
        result = train_and_evaluate_one(X_3d, y, feat_group, group_dir, SPOTCHECK_SUBJECT, tag="P4_Spotcheck")
        spotcheck_results[feat_group] = result

        with open(os.path.join(group_dir, f"results_{feat_group}.json"), "w") as f:
            json.dump(result, f, indent=2)

        print(f"[INFO][P4-A] {feat_group:<10} test acc {result['test_accuracy']*100:6.2f}%  "
...
def run_stage_b_fullscale(spotcheck_results=None, subject_ids=None):
    """Full-scale training loop. `subject_ids`, if given, restricts the run
    to a subset (used for smoke-testing and for manual single-subject runs);
    the default (None) auto-discovers and runs all 12 subjects unattended."""
    print(f"\n{'=' * 70}\n P4 No-Windowing -- Stage B: Full-scale ({EXP_ID})\n{'=' * 70}")
    ...
    if spotcheck_results is None:
        ...
        if not os.path.exists(summary_path):
            raise RuntimeError("[P4 Stage B] Stage A spot-check summary not found -- run Stage A first.")
    ...
    print(f"[INFO][P4-B] Auto-selected feature group: {winning_group}  ({selection['reason']})")
    ...
    for subject_id in subject_ids:
        model_path = build_model_path(stage_weights_dir, "P4_Fullscale", winning_group, subject_id)
        if os.path.exists(model_path):
            print(f"[SKIP][P4-B] Model for {subject_id} already exists.")
        ...
        X_3d, y = load_no_window_epochs(subject_id, raw_dir)
        if X_3d is None:
            print(f"[WARNING][P4-B] No epochs extracted for subject {subject_id}; skipping.")
            skipped_no_data.append(subject_id)
            continue

        result = train_and_evaluate_one(X_3d, y, winning_group, stage_weights_dir, subject_id, tag="P4_Fullscale")
        fullscale_results[subject_id] = result

        with open(os.path.join(stage_weights_dir, f"results_{subject_id}.json"), "w") as f:
            json.dump(result, f, indent=2)

        print(f"[INFO][P4-B] {subject_id}: test acc {result['test_accuracy']*100:6.2f}%  "
...
def write_report(spotcheck_results, selection, fullscale_results, all_subject_ids, skipped_no_data):
    ...
    lines = []
    lines.append("# P4 -- No-Windowing: Experiment Report")
    lines.append("")
    lines.append("Variable tested: epoch length (one full 5-second epoch vs. the standard "
                  "five 1-second windows). Locked: 0.5-50Hz broadband filter, SVM, "
                  "subject-dependent architecture, E0 Baseline (no augmentation), phase_filter='all'.")
    lines.append("")
    if n_complete < n_total:
        lines.append(f"> **Status: PARTIAL ({n_complete}/{n_total} subjects).** This report reflects a "
                      ...
                      f"`run_p4_nowindowing.py --stage b` on the lab machine to complete the grid; "
    ...
    lines.append("")
    lines.append("## Prior pilot context")
    lines.append("")
    lines.append("A same-day prior pilot (S3 only, Barlow only) is preserved at "
                  "`backend/models/weights/P4_NoWindowing/E0_Baseline/` -- see "
                  "`backend/reports/p4_no_windowing_pilot_report.md` (0% test accuracy at n=106) and "
                  "`p4_control_subsampled_report.md` (matched-size subsampling control, mean ~6.09% "
                  "test accuracy, attributing the pilot's 0% mainly to small sample size, z~-1.57, "
                  "not significant). This report's Stage A `barlow`/S3 result is a useful consistency "
                  "check against that pilot, since both use the same data, split seed, and model config.")

    with open(REPORT_PATH, "w", encoding="utf-8") as f:
        f.write("\n".join(lines) + "\n")
    print(f"[INFO][P4] Report written to {REPORT_PATH}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="P4 No-Windowing: spot-check + full-scale SVM grid.")
