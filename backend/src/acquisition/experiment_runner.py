...
try:
    ...
    # Default parameters matching the thesis acquisition protocol
...
class ExperimentRunner:
    def __init__(self, subject_id, logger):
        ...
        try:
            ...
            self.logger.info("[INFO] Audio loaded successfully.")
        except Exception as e:
            self.logger.warning(f"[WARNING] Failed to load audio: {e}. Expected path: {path_bip}")
            self.sound_bip = None
            self.sound_double_bip = None

        self.logger.info("[INFO] Establishing LSL Marker stream...")
        ...
        self.logger.info("[INFO] LSL Marker outlet ready.")
    ...
    def draw_trial_screen(self, main_text, sub_text="", tracker_info=""):
        ...
        signal_surf = self.font_tracker.render("LSL Marker: ACTIVE", True, (50, 200, 50))
    ...
    def run_trial(self, word, trial_number, phase_name, block_number):
        ...
        # Visual cue: display the full target word before the syllable slots begin
        self.custom_wait(1.5, word.upper(), "TARGET WORD", tracker_text)

        # Slot 1 (t=0–5s): first syllable, auditory cue, LSL marker injection
        self.play_sound(self.sound_bip)
        self.lsl_outlet.push_sample([id_syl1])
        self.logger.info(f"Marker injected — Slot 1: {syl1} (ID: {id_syl1})")
        self.custom_wait(SLOT_1_DURATION, syl1.upper(), f"Ulangi Suku Kata Ini ({phase_name})", tracker_text)

        # Inter-slot pause (t=5–7s): blank screen to reset mental state
        self.custom_wait(PAUSE_DURATION, "", "", tracker_text)

        # Slot 2 (t=7–12s): second syllable, auditory cue, LSL marker injection
        self.play_sound(self.sound_bip)
        self.lsl_outlet.push_sample([id_syl2])
        self.logger.info(f"Marker injected — Slot 2: {syl2} (ID: {id_syl2})")
        self.custom_wait(SLOT_2_DURATION, syl2.upper(), f"Ulangi Suku Kata Ini ({phase_name})", tracker_text)

        # End of trial: double beep and brief ITI before the next trial
    ...
    def start_experiment(self):
        self.logger.info("[INFO] Starting experiment protocol...")
...
if __name__ == "__main__":
    print("=" * 50)
    print(" NEURANDIAR BCI — EEG DATA ACQUISITION SYSTEM ")
    print("=" * 50)
